...
async function run() {
    ...
    console.log(`Found ${targetCompanies.length} companies with unlock events in This Month / Last Month.`);
    ...
    for (let i = 0; i < targetCompanies.length; i++) {
        ...
        try {
            ...
            if (detailRes.data.fromCache) {
                ...
                skipped++;
            } else if (detailRes.data.error) {
                console.log(` -> Fetch Error: ${detailRes.data.error}`);
                failed++;
            } else {
